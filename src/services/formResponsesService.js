...
export async function loadFormResponsesAsync(){const {hydrateFormsBackend}=await import('./backend/configurationBackendService');return (await hydrateFormsBackend()).responses}
export async function upsertFormResponseAsync(record){const {saveFormResponseBackend}=await import('./backend/configurationBackendService');return saveFormResponseBackend(record)}
export async function deleteFormResponseAsync(id){const {deleteFormResponseBackend}=await import('./backend/configurationBackendService');return deleteFormResponseBackend(id)}
