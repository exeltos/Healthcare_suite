...
import { mapPromotedAntibiotic, mapEmployeeVaccination, mapWasteMeasurement, mapAntisepticConsumption, mapHandHygieneSession } from './preventionBackendMappers'
import {
...
export async function loadWasteMeasurementsRelational(){
 ...
 if(error)throw error
 const rows=(data||[]).map(mapWasteMeasurement)
 withProductionCacheWrite(()=>saveWasteMeasurements(rows))
 return rows
}
