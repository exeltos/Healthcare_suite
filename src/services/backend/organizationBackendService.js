...
export async function loadOperationalDocuments(){
  ...
  const {data,error}=await c.from('controlled_documents').select('*').order('title')
  if(error)throw error
...
export async function saveOperationalDocument(input={}){
  ...
  const c=requireSupabase(),org=await orgId(c),actor=await actorId(c),row={...input,id:input.id||`DOC-${Date.now()}`}
  ...
  const normalizedAttachments=await syncDocumentAttachments(c,{org,actor,documentId:String(row.id),attachments:Array.isArray(row.attachments)?row.attachments:[]})
  const legacyVersions=Array.isArray(row.versions)?row.versions:[]
  const versionKeys=legacyVersions.map(v=>String(v.version||'').trim()).filter(Boolean)
  ...
  const payload={
    id:String(row.id),organization_id:org,title:String(row.title||''),code:String(row.code||''),category:String(row.category||''),
    version:String(row.version||''),owner:String(row.owner||''),status:String(row.status||'Σε ισχύ'),review_date:date(row.reviewDate),
    attachments:normalizedAttachments.map(stripDocumentAttachmentForJson),
    versions:legacyVersions,
    data:rest(row,['id','title','code','category','version','owner','status','reviewDate','attachments','versions','createdAt','updatedAt'])
  }
  const {data:saved,error}=await c.from('controlled_documents').upsert(payload,{onConflict:'id'}).select().single()
  if(error)throw error

  if(row.status==='Σε ισχύ'){
    await ensureControlledDocumentVersion(c,{org,actor,row:{...row,attachments:normalizedAttachments}})
  }

  const hydrated=await hydrateRelationalDocuments(c,[mapDocument(saved)])
  return hydrated[0]
}
export async function deleteOperationalDocument(id){
  if(!IS_PRODUCTION)return deleteDocument(id)
  const c=requireSupabase()
  const {data:row,error:readError}=await c.from('controlled_documents').select('status,organization_id').eq('id',String(id)).single()
  if(readError)throw readError
  if(row?.status!=='Πρόχειρο')throw new Error('Only draft controlled documents can be deleted.')
  const {data:files,error:filesError}=await c.from('attachments').select('*').eq('entity_type','controlled_document').eq('entity_id',String(id))
  if(filesError)throw filesError
  if((files||[]).length){
    const paths=(files||[]).filter(x=>x.bucket==='operationalattachments').map(x=>x.object_path)
    if(paths.length){
      const {error:storageError}=await c.storage.from('operationalattachments').remove(paths)
      if(storageError)throw storageError
    }
    const {error:metaError}=await c.from('attachments').delete().eq('entity_type','controlled_document').eq('entity_id',String(id))
