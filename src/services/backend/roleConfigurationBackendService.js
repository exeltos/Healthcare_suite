...
export async function saveRoleConfiguration(rows=[]){
 ...
 if(!Array.isArray(rows)||rows.length!==defaults.length||rows.some(row=>!allowedModules.has(row.module)))throw new Error('Invalid role permission configuration.')
 for(const row of rows){for(const role of allowedRoles){if(typeof row[role]!=='string'||!row[role].trim())throw new Error('Every role must have an explicit access level for every module.')}}
