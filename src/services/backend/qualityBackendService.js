...
import { loadIncidents,saveIncidents,loadCapa,saveCapa,loadAuditExecutions,saveAuditExecutions,upsertAuditExecution,deleteAuditExecution } from '../qualityService'
...
export async function deleteQualityIncident(id){
  if(!IS_PRODUCTION){saveIncidents(loadIncidents().filter(x=>String(x.id)!==String(id)));return true}
  const c=requireSupabase();const {error}=await c.from('quality_incidents').delete().eq('id',String(id));if(error)throw error;return true
}
...
export async function deleteQualityCapa(id){
  if(!IS_PRODUCTION){saveCapa(loadCapa().filter(x=>String(x.id)!==String(id)));return true}
  const c=requireSupabase();const {error}=await c.from('quality_capa').delete().eq('id',String(id));if(error)throw error;return true
}
...
export async function saveQualityAudit(input={}){
  if(!IS_PRODUCTION)return upsertAuditExecution(input)
...
export async function deleteQualityAudit(id){
  if(!IS_PRODUCTION)return deleteAuditExecution(id)
  const c=requireSupabase();const {error}=await c.from('quality_audits').delete().eq('id',String(id));if(error)throw error;return true
}

function mapIncident(r){return {...r.data,id:r.id,date:r.incident_date||'',time:String(r.incident_time||'').slice(0,5),title:r.title,category:r.category,department:one(r.department)?.name||'',outcome:r.outcome,status:r.status,owner:r.owner,description:r.description,createdAt:r.created_at,updatedAt:r.updated_at}}
function mapCapa(r){return {...r.data,id:r.id,title:r.title,source:r.source_id,sourceType:r.source_type,parentId:r.parent_id,actionType:r.action_type,owner:r.owner,department:one(r.department)?.name||'',dueDate:r.due_date||'',priority:r.priority,progress:Number(r.progress||0),status:r.status,description:r.description,rootCause:r.root_cause,plannedAction:r.planned_action,evidence:r.evidence,effectivenessStatus:r.effectiveness_status,effectivenessDate:r.effectiveness_date||'',effectivenessNotes:r.effectiveness_notes,createdAt:r.created_at,updatedAt:r.updated_at}}
function mapAudit(r){return {...r.data,id:r.id,department:one(r.department)?.name||'',templateId:r.template_id||'',templateName:r.template_name,date:r.audit_date||'',status:r.status,compliance:r.compliance==null?'':Number(r.compliance),scope:r.scope,owner:r.owner,source:r.source_id,findings:Array.isArray(r.findings)?r.findings:[],answers:Array.isArray(r.answers)?r.answers:[],createdAt:r.created_at,updatedAt:r.updated_at}}
