...
export async function saveClinicalSourceSample(input={}){
  ...
  const payload={id:String(row.id),organization_id:org,source_type:row.sourceType,employee_id:employeeId,department_id:dep,
    ...
    data:rest(row,['id','sourceType','department','subjectName','subjectCode','sampleType','sampleReason','collectionDate','collectionTime','receivedDate','resultDate','status','resultStatus','microorganism','resistance'])}
