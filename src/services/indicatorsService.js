...
import { loadStaffVaccinations, loadPromotedAntibiotics } from './preventionService'
...
const OFFICIAL_DEFINITIONS = [
  ...
  { id:'restricted-antibiotic-approval', name:'Έγκριση Προωθημένων Αντιβιοτικών', category:'Antimicrobial Stewardship', unit:'%', frequency:'Μηνιαία', recipient:'ΕΝΛ · Επιτροπή Φαρμάκων · Ποιότητα', source:'Αντιμικροβιακή Επιτήρηση · Προωθημένα Αντιβιοτικά', formula:'(Εγκεκριμένα αιτήματα ÷ αιτήματα με τελική απόφαση) × 100', mode:'automatic', description:'Παρακολουθεί το ποσοστό έγκρισης των αιτημάτων περιορισμένης χρήσης. Τα εκκρεμή αιτήματα εμφανίζονται χωριστά και δεν περιλαμβάνονται στον παρονομαστή.' },
  { id:'restricted-antibiotic-pending', name:'Εκκρεμή Προωθημένα Αντιβιοτικά', category:'Antimicrobial Stewardship', unit:'αιτήματα', frequency:'Συνεχής', recipient:'ΕΝΛ · Εγκρίνοντες ιατροί', source:'Αντιμικροβιακή Επιτήρηση · Προωθημένα Αντιβιοτικά', formula:'Αριθμός αιτημάτων περιορισμένης χρήσης που παραμένουν σε κατάσταση Εκκρεμεί', mode:'automatic', description:'Closed-loop operational indicator για αιτήματα που δεν έχουν ακόμη λάβει τελική ιατρική απόφαση.' },
...
function restrictedAntibioticApprovalMetric(custom){
  const range=resolvedRange(custom,currentMonthRange())
  const rows=loadPromotedAntibiotics().filter(row=>inRange(row.date||row.approvalDate||row.createdAt,range.start,range.end))
  const decided=rows.filter(row=>['Εγκρίθηκε','Απορρίφθηκε'].includes(row.approval))
  const approved=decided.filter(row=>row.approval==='Εγκρίθηκε').length
  const pending=rows.filter(row=>row.approval==='Εκκρεμεί').length
  return withPeriod({value:percent(approved,decided.length),numerator:approved,denominator:decided.length||null,history:[],inputs:[input('Αιτήματα περιόδου',rows.length),input('Εγκεκριμένα',approved),input('Απορριφθέντα',decided.length-approved),input('Εκκρεμή',pending)]},range)
}
function restrictedAntibioticPendingMetric(custom){
  const range=resolvedRange(custom,currentMonthRange())
  const rows=loadPromotedAntibiotics().filter(row=>inRange(row.date||row.createdAt,range.start,range.end))
  const pending=rows.filter(row=>row.approval==='Εκκρεμεί').length
  return withPeriod({value:pending,numerator:pending,denominator:null,history:[],inputs:[input('Αιτήματα περιόδου',rows.length),input('Εκκρεμή',pending)]},range)
}
