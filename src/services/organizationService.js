...
export const loadTraining=()=>organizationRepository.findTraining(IS_PRODUCTION?[]:seedTraining).map(x=>({...x,attendance:x.attendance||[],attachments:x.attachments||[],durationHours:x.durationHours||0}))
...
export const loadCommittees=()=>organizationRepository.findCommittees(IS_PRODUCTION?[]:seedCommittees).map(x=>({...x,memberIds:x.memberIds||[],members:x.members||[],agenda:x.agenda||[],meetings:(x.meetings||[]).map(m=>({...m,presentIds:m.presentIds||[],actions:m.actions||[],attachments:m.attachments||[]})),attachments:x.attachments||[]}))
