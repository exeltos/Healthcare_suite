...
export default function EmployeesPage() {
  ...
  return (
    <PageChrome
      ...
      header={<PageHeader title={L("Εργαζόμενοι", "Employees")} description={L("Κεντρικό μητρώο προσωπικού", "Central staff registry")} actions={<Button icon={<Plus size={17} />} onClick={openCreateEmployee}>{L('Νέος εργαζόμενος', 'New employee')}</Button>} />}
    >
      <ListWorkspace
      ...
      <Drawer
        ...
        title={L("Νέος εργαζόμενος", "New employee")}
        width={900}
        position="center"
      ...
      >
        <Form id="employee-create-form" className="employee-create-form" onSubmit={createEmployee} onCancel={closeCreateEmployee} isDirty={employeeFormDirty}>
          <section><h3>{L("Προσωπικά στοιχεία", "Personal details")}</h3><div className="employee-create-grid"><label><span>{L("Επώνυμο *", "Last name *")}</span><input required aria-invalid={Boolean(formErrors.lastName)} value={form.lastName} onChange={(e) => setEmployeeField('lastName', e.target.value)} />{formErrors.lastName && <small className="core-field__error">{formErrors.lastName}</small>}</label><label><span>{L("Όνομα *", "First name *")}</span><input required aria-invalid={Boolean(formErrors.firstName)} value={form.firstName} onChange={(e) => setEmployeeField('firstName', e.target.value)} />{formErrors.firstName && <small className="core-field__error">{formErrors.firstName}</small>}</label><label><span>{L("Πατρώνυμο", "Father’s name")}</span><input value={form.fatherName} onChange={(e) => setEmployeeField('fatherName', e.target.value)} /></label><label><span>{L("Φύλο", "Sex")}</span><select value={form.gender || ''} onChange={(e) => setEmployeeField('gender', e.target.value)}><option value="">{L("Επιλέξτε", "Select")}</option><option value="Άνδρας">{employeeDisplayValue("Άνδρας", language)}</option><option value="Γυναίκα">{employeeDisplayValue("Γυναίκα", language)}</option><option value="Άλλο / μη δηλωμένο">{employeeDisplayValue("Άλλο / μη δηλωμένο", language)}</option></select></label></div></section>
          <section><h3>{L("Υπηρεσιακά στοιχεία", "Employment details")}</h3><div className="employee-create-grid"><label><span>{L("Κωδικός", "Code")}</span><input value={form.employeeCode} onChange={(e) => setEmployeeField('employeeCode', e.target.value)} /></label><LibraryField label={L("Τμήμα", "Department")} libraryKey="departments" value={form.department} onChange={(value) => setEmployeeField('department', value)} /><LibraryField label={L("Ιδιότητα", "Professional category")} libraryKey="professional-categories" value={form.professionalCategory} onChange={(value) => setEmployeeField('professionalCategory', value)} /><label><span>{L("Ημερομηνία πρόσληψης", "Hire date")}</span><input type="date" value={form.hireDate} onChange={(e) => setEmployeeField('hireDate', e.target.value)} /></label><label><span>{L("Κατάσταση", "Status")}</span><select value={form.status} onChange={(e) => setEmployeeField('status', e.target.value)}><option value="Ενεργό">{employeeDisplayValue("Ενεργό", language)}</option><option value="Ανενεργό">{employeeDisplayValue("Ανενεργό", language)}</option></select></label></div></section>
          <section><h3>{L("Επικοινωνία", "Contact")}</h3><div className="employee-create-grid"><label><span>Email</span><input type="email" aria-invalid={Boolean(formErrors.email)} value={form.email} onChange={(e) => setEmployeeField('email', e.target.value)} />{formErrors.email && <small className="core-field__error">{formErrors.email}</small>}</label><label><span>{L("Τηλέφωνο", "Phone")}</span><input value={form.phone} onChange={(e) => setEmployeeField('phone', e.target.value)} /></label></div></section>
        </Form>
      </Drawer>
