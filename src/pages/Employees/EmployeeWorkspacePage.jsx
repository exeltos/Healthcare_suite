...
export default function EmployeeWorkspacePage() {
  ...
  async function saveVaccinationRecord(event) {
    ...
    try {
      ...
      const persisted = refreshed.some((row) => String(row.id) === String(saved.id) && String(row.employeeId || '') === String(employee.id))
      if (!persisted) throw new Error(L('Ο εμβολιασμός δεν επιβεβαιώθηκε στο Supabase.', 'Vaccination could not be verified in Supabase.'))
      setVaccinations(refreshed)
      setLastVaccinationId(saved.id)
      setSelectedVaccination(null)
      notifyAction(L('Ο εμβολιασμός αποθηκεύτηκε στο Supabase.', 'Vaccination saved to Supabase.'))
    } catch (error) {
      console.error('Vaccination save failed', error)
      notifyAction(String(error?.message || '').trim() || L('Η αποθήκευση του εμβολιασμού απέτυχε.', 'Vaccination save failed.'))
    }
  }
  ...
  async function saveOccupationalVisit(event) {
    ...
    try {
      ...
      const refreshed = await loadEmployeeOccupationalVisits(employee.id)
      const persisted = refreshed.some((row) => String(row.id) === String(saved.id))
      if (!persisted) throw new Error(L('Η επίσκεψη δεν επιβεβαιώθηκε στο Supabase.', 'Occupational-health visit could not be verified in Supabase.'))
      setOccupationalVisitsState(refreshed)
      setOccupationalDraft(null)
      notifyAction(L('Η επίσκεψη στον Ιατρό Εργασίας αποθηκεύτηκε στο Supabase.', 'Occupational-health visit saved to Supabase.'))
    } catch (error) {
      console.error('Occupational health save failed', error)
      notifyAction(String(error?.message || '').trim() || L('Η αποθήκευση της επίσκεψης απέτυχε.', 'Occupational-health visit save failed.'))
    }
  }
