      ...
      {view === 'vaccinations' && (selectedVaccination ? <section className="ew-list-section ew-vaccine-detail">
        ...
        <form className="ew-vaccine-form" onSubmit={onSaveVaccination}>
          <div className="ew-grid ew-grid--three">
            ...
            <Field label={L('Ημερομηνία *','Date *')}><EmployeeDateInput value={selectedVaccination.date || ''} onChange={(value) => setSelectedVaccination({ ...selectedVaccination, date: value })} /></Field>
            ...
            <Field label={L('Ισχύς έως','Valid until')}><EmployeeDateInput value={selectedVaccination.validUntil || ''} onChange={(value) => setSelectedVaccination({ ...selectedVaccination, validUntil: value })} /></Field>
          ...
          <div className="ew-form-actions ew-form-actions--split">
            <div>{selectedVaccination.id && <Button variant="danger" type="button" onClick={() => onDeleteVaccination?.(selectedVaccination.id)}>{L('Διαγραφή','Delete')}</Button>}</div>
            <div className="ew-form-actions-group"><Button variant="secondary" type="button" onClick={() => setSelectedVaccination(null)}>{L('Ακύρωση','Cancel')}</Button><Button type="submit" icon={<Save size={15}/>}>{L('Αποθήκευση','Save')}</Button></div>
          </div>
        </form>
  ...
  return <div className="ew-profile">
    ...
    <section className="ew-section">
      ...
      <div className="ew-grid ew-grid--three">
        ...
        <Field label={L('Ημερομηνία πρόσληψης', 'Hire date')}><EmployeeDateInput disabled={!editing} value={form.hireDate || ''} onChange={(value) => setForm({ ...form, hireDate: value })} /></Field>
...
function OccupationalHealthTab({ language, visits, draft, setDraft, onSave, onDelete }) {
  ...
  return <section className="ew-list-section ew-occupational-list">
    ...
    {isEditing ? <form className="ew-occupational-form" onSubmit={onSave}>
      <div className="ew-grid ew-grid--three">
        <Field label={L('Ημερομηνία εξέτασης *', 'Examination date *')}><EmployeeDateInput value={draft.date || ''} onChange={(value) => setDraft({ ...draft, date: value })} /></Field>
        ...
        <Field label={L('Επόμενος επανέλεγχος', 'Next review')}><EmployeeDateInput value={draft.nextReviewDate || ''} onChange={(value) => setDraft({ ...draft, nextReviewDate: value })} /></Field>
        <Field label={L('Σημειώσεις', 'Notes')} wide><textarea value={draft.notes || ''} onChange={(e) => setDraft({ ...draft, notes: e.target.value })} /></Field>
      </div>
      <div className="ew-form-actions ew-form-actions--split">
        <div>{draft.id && <Button variant="danger" type="button" onClick={() => onDelete(draft.id)}>{L('Διαγραφή', 'Delete')}</Button>}</div>
        <div className="ew-form-actions-group"><Button variant="secondary" type="button" onClick={() => setDraft(null)}>{L('Ακύρωση', 'Cancel')}</Button><Button type="submit" icon={<Save size={15}/>}>{L('Αποθήκευση', 'Save')}</Button></div>
      </div>
...
function isoToEmployeeDate(value = '') {
  const match = String(value || '').match(/^(\d{4})-(\d{2})-(\d{2})$/)
  return match ? `${match[3]}/${match[2]}/${match[1]}` : String(value || '')
}

function employeeDateToIso(value = '') {
  ...
  if (Number.isNaN(probe.getTime()) || probe.getDate() !== Number(day) || probe.getMonth() + 1 !== Number(month)) return null
  return iso
}

function EmployeeDateInput({ value = '', onChange, disabled = false }) {
  const display = isoToEmployeeDate(value)
  return <input
    className="ew-date-input"
    type="text"
    inputMode="numeric"
    autoComplete="off"
    disabled={disabled}
    ...
    value={display}
    onChange={(event) => {
      const text = event.target.value.replace(/[^0-9\/.-]/g, '').slice(0, 10)
      const iso = employeeDateToIso(text)
      if (iso !== null) onChange?.(iso)
      else if (!text) onChange?.('')
      else {
        // Keep partial typing locally by passing the display string; it will normalize once complete.
        onChange?.(text)
      }
    }}
    onBlur={(event) => {
      const iso = employeeDateToIso(event.target.value)
      if (iso !== null) onChange?.(iso)
    }}
  />
}
