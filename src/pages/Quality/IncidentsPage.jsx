...
const STATUSES = ['Νέα αναφορά', 'Υπό διερεύνηση', 'Σε ενέργειες βελτίωσης', 'Κλειστό', 'Ακυρωμένο']
...
export default function IncidentsPage(){
  ...
  const metrics=useMemo(()=>({total:filtered.length,open:filtered.filter(r=>!['Κλειστό','Ακυρωμένο'].includes(r.status)).length,severe:filtered.filter(r=>['Σοβαρή βλάβη','Θάνατος'].includes(r.outcome)).length,nearMiss:filtered.filter(r=>r.outcome==='Χωρίς βλάβη / near miss').length}),[filtered])
  ...
  async function save(e){e.preventDefault();let nextStatus=form.status;if(editing&&nextStatus==='Νέα αναφορά'&&(form.owner||form.contributingFactors||form.rootCause))nextStatus='Υπό διερεύνηση';const record={...form,status:nextStatus,id:editing?.id||`INC-${Date.now().toString().slice(-7)}`,updatedAt:new Date().toISOString(),createdAt:editing?.createdAt||new Date().toISOString()};await saveQualityIncident(record);setRows(await loadQualityIncidents());close()}
  async function remove(){if(!editing)return;const reason=promptAction(L('Σύντομος λόγος ακύρωσης της εγγραφής:','Short reason for cancelling this record:'));if(reason===null)return;if(!String(reason).trim()){notifyAction(L('Χρειάζεται ένας σύντομος λόγος ακύρωσης.','A short cancellation reason is required.'));return}await deleteQualityIncident(editing.id,String(reason).trim());setRows(await loadQualityIncidents());close()}
  ...
  async function startInvestigation(){if(!editing||form.status!=='Νέα αναφορά')return;const updated={...form,status:'Υπό διερεύνηση'};await saveQualityIncident({...editing,...updated});setForm(updated);setRows(await loadQualityIncidents())}
  ...
  return <PageChrome className="quality-unified-page" back={<BackLink onClick={()=>navigate(APP_ROUTES.QUALITY)}>{L('Πίσω στο Quality Hub','Back to Quality Hub')}</BackLink>} header={<PageHeader title={L('Συμβάντα Ασφάλειας','Safety Incidents')} description={L('Αναφορά, διερεύνηση και παρακολούθηση συμβάντων ασφάλειας ασθενών με δομημένη ροή.','Report, investigate and follow patient-safety incidents through a structured workflow.')} actions={<Button icon={<Plus size={17}/>} onClick={openNew}>{L('Νέο συμβάν','New incident')}</Button>}/> }>
    ...
    <Drawer open={drawerOpen} onClose={close} title={editing?L('Επεξεργασία συμβάντος','Edit incident'):L('Νέο συμβάν ασφάλειας','New safety incident')} description={L('Καταγράψτε τα πραγματικά γεγονότα, την έκβαση και τις ενέργειες χωρίς απόδοση υπαιτιότητας.','Record factual events, outcome and actions without assigning blame.')} width={1120} position="center" footer={<FormActions form="incident-form" onCancel={close} disabled={Boolean(editing?.archivedAt)} primaryLabel={editing?.archivedAt?L('Ακυρωμένο','Cancelled'):undefined} extraActions={editing&&!editing.archivedAt?<div className="quality-inline-actions">{form.status==='Νέα αναφορά'&&<Button variant="secondary" onClick={startInvestigation}>{L('Έναρξη διερεύνησης','Start investigation')}</Button>}<Button variant="secondary" icon={<ClipboardCheck size={16}/>} onClick={createAudit}>{L('Δημιουργία Audit','Create Audit')}</Button><Button variant="secondary" onClick={createCapa}>{L('Δημιουργία CAPA','Create CAPA')}</Button>{!['Κλειστό','Ακυρωμένο'].includes(form.status)&&<Button variant="secondary" onClick={closeIncident}>{L('Κλείσιμο συμβάντος','Close incident')}</Button>}<Button variant="danger" icon={<Trash2 size={16}/>} onClick={remove}>{L('Ακύρωση εγγραφής','Cancel record')}</Button></div>:null}/> }>
      <form id="incident-form" className="quality-unified-form" onSubmit={save}><fieldset disabled={Boolean(editing?.archivedAt)} className="quality-governance-fieldset">
        ...
        <FormSection title={L('Διερεύνηση','Investigation')}><FormGrid columns={2}><FormField label={L('Υπεύθυνος διερεύνησης','Investigation owner')}><input value={form.owner||''} onChange={e=>setField('owner',e.target.value)}/></FormField><FormField label={L('Κατάσταση','Status')}><div className="quality-risk-score"><Badge tone={form.status==='Κλειστό'?'success':form.status==='Υπό διερεύνηση'?'warning':'info'}>{qualityDisplayValue(form.status,language)}</Badge></div></FormField></FormGrid><FormField label={L('Συνεισφέροντες παράγοντες','Contributing factors')}><textarea rows="4" value={form.contributingFactors||''} onChange={e=>setField('contributingFactors',e.target.value)}/></FormField><FormField label={L('Βασική αιτία / συμπέρασμα διερεύνησης','Root cause / investigation conclusion')}><textarea rows="4" value={form.rootCause||''} onChange={e=>setField('rootCause',e.target.value)}/></FormField><FormField label={L('Μάθηση / μέτρα πρόληψης επανάληψης','Lessons / recurrence prevention')}><textarea rows="4" value={form.lessonsLearned||''} onChange={e=>setField('lessonsLearned',e.target.value)}/></FormField></FormSection>
      </fieldset></form>
    </Drawer>
  </PageChrome>
}