...
export default function PlatformOwnerPage(){
 ...
 const [editing,setEditing]=useState(null),[editName,setEditName]=useState(''),[editSlug,setEditSlug]=useState(''),[rowBusy,setRowBusy]=useState('')
 ...
 function startEdit(row){setError('');setMessage('');setEditing(row.id);setEditName(row.name);setEditSlug(row.slug)}
 function cancelEdit(){if(rowBusy)return;setEditing(null);setEditName('');setEditSlug('')}
 async function saveEdit(row){
  if(rowBusy)return;const normalizedSlug=makeSlug(editSlug||editName);if(!editName.trim()||!normalizedSlug){setError('Συμπλήρωσε έγκυρο όνομα και κωδικό νοσοκομείου.');return}
  setRowBusy(row.id);setError('');setMessage('')
  try{await updatePlatformOrganization(row.id,{name:editName.trim(),slug:normalizedSlug});setEditing(null);setMessage('Τα στοιχεία του νοσοκομείου ενημερώθηκαν.');await reload()}
  catch(e){setError(e.message)}finally{setRowBusy('')}
 }
 async function remove(row){
  if(rowBusy)return
  if(!window.confirm(`Να διαγραφεί το «${row.name}»; Η διαγραφή επιτρέπεται μόνο όταν δεν υπάρχουν χρήστες ή συνδεδεμένα δεδομένα.`))return
  setRowBusy(row.id);setError('');setMessage('')
  try{await deletePlatformOrganization(row.id);if(editing===row.id)cancelEdit();setMessage('Το κενό νοσοκομείο διαγράφηκε.');await reload()}
  catch(e){setError(e.message)}finally{setRowBusy('')}
 }
 async function toggle(row){try{setRowBusy(row.id);setError('');await setPlatformOrganizationActive(row.id,!row.active);await reload()}catch(e){setError(e.message)}finally{setRowBusy('')}}
