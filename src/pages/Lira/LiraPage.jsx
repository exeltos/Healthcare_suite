...
export default function LiraPage(){
 ...
 return <PageChrome className="lira-page" header={<PageHeader eyebrow="LIRA AI" title="LIRA — Healthcare Intelligence Assistant" description={L('Εξηγήσιμη υποστήριξη επιτήρησης, ποιότητας και διοικητικής ανάλυσης πάνω στα πραγματικά δεδομένα της εφαρμογής.','Explainable surveillance, quality and operational decision support over the application’s actual data.')} actions={<Button type="button" className="lira-refresh" icon={<RefreshCw size={15}/>} onClick={refresh}>{L('Ανανέωση','Refresh')}</Button>}/> }>
  ...
  <div className="lira-data-quality"><strong>{L('Ποιότητα δεδομένων','Data quality')}</strong><span>{d.dataQuality.denominatorReady?L('Διαθέσιμοι παρονομαστές ασθενών-ημερών.','Patient-day denominators available.'):L('Χωρίς ασθενείς-ημέρες: οι σχετικοί ρυθμοί δεν είναι συγκρίσιμοι.','Patient-days unavailable: related rates are not comparable.')}</span>{d.dataQuality.indicatorMissing>0&&<span>{L(`${d.dataQuality.indicatorMissing} δείκτες χωρίς επαρκή δεδομένα.`,`${d.dataQuality.indicatorMissing} indicators lack sufficient data.`)}</span>}</div>
...
function Insight({id,count,path,tone='neutral',L}){const copy=signalCopy(id,count,L);return <article className={`lira-insight ${tone}`}><div><strong>{copy.title}</strong><p>{copy.text}</p><small>{copy.basis}</small></div><Link to={path}>{L('Άνοιγμα πηγής','Open source')}<ArrowRight size={13}/></Link></article>}
function signalCopy(id,count,L){const map={
 mdr:[L('MDR/XDR εργαστηριακά ευρήματα','MDR/XDR laboratory findings'),L(`${count} θετικά ευρήματα έχουν σήμανση αντοχής.`,`${count} positive findings are flagged for resistance.`),L('Βάση: θετικό αποτέλεσμα + MDR/XDR.','Basis: positive result + MDR/XDR flag.')],
 capa:[L('Εκπρόθεσμες CAPA','Overdue CAPA'),L(`${count} ανοικτές CAPA έχουν περάσει την προθεσμία.`,`${count} open CAPA are past due.`),L('Βάση: ανοικτή κατάσταση + ληγμένη προθεσμία.','Basis: open status + overdue due date.')],
 risk:[L('Υψηλός υπολειπόμενος κίνδυνος','High residual risk'),L(`${count} κίνδυνοι παραμένουν υψηλοί.`,`${count} risks remain high.`),L('Βάση: residual score ≥ 12 και μη κλειστή εγγραφή.','Basis: residual score ≥ 12 and record not closed.')],
 abx:[L('Εκκρεμείς εγκρίσεις αντιβιοτικών','Pending antimicrobial approvals'),L(`${count} αιτήματα αναμένουν απόφαση.`,`${count} requests await a decision.`),L('Βάση: κατάσταση έγκρισης Εκκρεμεί.','Basis: approval status is Pending.')],
 docs:[L('Έγγραφα προς αναθεώρηση','Documents due for review'),L(`${count} έγγραφα χρειάζονται αναθεώρηση.`,`${count} documents require review.`),L('Βάση: σε ισχύ και ληγμένη/εντός 30 ημερών αναθεώρηση.','Basis: effective and review overdue/due within 30 days.')],
 training:[L('Επανεκπαίδευση / επάρκεια','Retraining / competency'),L(`${count} εγγραφές χρειάζονται επανέλεγχο.`,`${count} records require review.`),L('Βάση: ληγμένη επάρκεια ή ανάγκη επανεκπαίδευσης.','Basis: expired competency or retraining requirement.')],
};const x=map[id]||[id,String(count||''),''];return{title:x[0],text:x[1],basis:x[2]}}
...
function snapshot(){
 ...
 const signals=[]
 if(resistant.length)signals.push({id:'mdr',count:resistant.length,path:APP_ROUTES.LABORATORY,tone:'danger'})
 if(overdueCapa.length)signals.push({id:'capa',count:overdueCapa.length,path:APP_ROUTES.QUALITY_CAPA,tone:'danger'})
 if(highRisks.length)signals.push({id:'risk',count:highRisks.length,path:APP_ROUTES.QUALITY_RISKS,tone:'warning'})
 if(pendingRestricted.length)signals.push({id:'abx',count:pendingRestricted.length,path:APP_ROUTES.PROMOTED_ANTIBIOTICS,tone:'warning'})
 if(reviewDocs.length)signals.push({id:'docs',count:reviewDocs.length,path:APP_ROUTES.DOCUMENTS,tone:'warning'})
 if(trainingDue.length)signals.push({id:'training',count:trainingDue.length,path:APP_ROUTES.TRAINING,tone:'warning'})
 const dataQuality={denominatorReady:patientDays>0,indicatorMissing:indicators.filter(x=>x.metric?.value==null||x.metric?.value==='').length}
 return{patients,samples,positive,resistant,active,openCapa,overdueCapa,openIncidents,openFindings,highRisks,attentionIndicators,indicators,hh,ddd,dddTotal,dddRate,patientDays,pendingRestricted,training,trainingDue,documents,reviewDocs,signals,dataQuality,domainCount:8,generatedAt:new Date().toLocaleTimeString('el-GR',{hour:'2-digit',minute:'2-digit'})}
}
...
function answer(k,d,L,language='el'){
 ...
 const signalText=d.signals.length?d.signals.slice(0,4).map(x=>{const c=signalCopy(x.id,x.count,L);return `${c.title}: ${c.text}`}).join(' '):L('Δεν εντοπίζονται ενεργά σήματα προτεραιότητας στα διαθέσιμα δεδομένα.','No active priority signals are detected in the available data.')
