...
export function SampleEditor({ form, setForm, samples, save, cancel, files, upload, deleteAttachment, startRecheck }) {
  ...
  return <form className="pw-editor" onSubmit={save}><div className="pw-editor-head"><h3>{form.id ? L('Στοιχεία δείγματος', 'Sample details') : (repeat ? L('Νέος επανέλεγχος', 'New follow-up') : L('Νέο δείγμα', 'New sample'))}</h3><IconButton label={L("Κλείσιμο", "Close")} icon={<X size={16} />} onClick={cancel} /></div>
