...
import { AlertCircle, Archive, CheckCircle2, ChevronDown, ChevronUp, Copy, Download, FileCheck2, Files, Plus, Printer, RefreshCw, Send } from 'lucide-react'
import {
...
export default function DocumentsPage(){
 ...
 />}>
  ...
  <Drawer open={open} onClose={close} title={editing?L('Καρτέλα εγγράφου','Document record'):L('Νέο έγγραφο','New document')} description={editing?`${form.code||'—'} · ${L('Έκδοση','Version')} ${form.version||'—'}`:L('Δημιουργία ελεγχόμενου εγγράφου.','Create a controlled document.')} width={1120} position="center" footer={<FormActions form="document-form" onCancel={close} disabled={isLocked} primaryLabel={isLocked?L('Κλειδωμένη έκδοση','Locked version'):undefined} extraActions={editing?<div className="org-inline-actions">{editing.status==='Πρόχειρο'&&<Button variant="secondary" icon={<Send size={16}/>} onClick={()=>transition('Προς έγκριση')}>{L('Προς έγκριση','Send for approval')}</Button>}{editing.status==='Προς έγκριση'&&<Button icon={<CheckCircle2 size={16}/>} onClick={()=>transition('Σε ισχύ')}>{L('Έγκριση & θέση σε ισχύ','Approve & publish')}</Button>}{(editing.status==='Σε ισχύ'||state==='Προς αναθεώρηση')&&<Button variant="secondary" icon={<Copy size={16}/>} onClick={createRevision}>{L('Νέα έκδοση','New version')}</Button>}{editing.status==='Σε ισχύ'&&<Button variant="secondary" icon={<Archive size={16}/>} onClick={()=>transition('Καταργημένο')}>{L('Κατάργηση','Retire')}</Button>}</div>:null}/> }>
