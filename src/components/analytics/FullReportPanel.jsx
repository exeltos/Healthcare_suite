...
export default function FullReportPanel({samples=[],infections=[],isolations=[],language='el',onClose}){
  ...
  return <div className="full-report-overlay analytics-fullscreen-overlay"><section className="full-report-panel analytics-hub-panel analytics-fullscreen-panel">
    ...
    <div className="full-report-scroll analytics-hub-scroll">
      ...
      {tab==='antimicrobials'&&<div className="full-report-grid"><ChartSection title={en?'DDD by antimicrobial':'DDD ανά αντιμικροβιακό'}><PairBarList rows={data.antimicrobialDDD} yearA={yearA} yearB={yearB}/></ChartSection><ChartSection title={en?'Antimicrobial stewardship indicators':'Δείκτες αντιμικροβιακής επιτήρησης'}><div className="indicator-compare-grid">{data.indicators.filter(x=>['antibiotic-consumption','restricted-antibiotic-approval','restricted-antibiotic-pending'].includes(x.id)).map(x=><CompareCard key={x.id} label={`${x.label} · ${x.unit}`} a={x.a} b={x.b} yearA={yearA} yearB={yearB}/>)}</div></ChartSection></div>}
