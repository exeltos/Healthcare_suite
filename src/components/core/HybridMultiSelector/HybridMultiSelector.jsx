...
export default function HybridMultiSelector({
  ...
  function addManual() {
    const schema = Object.fromEntries(
      manualFields.filter((field) => field.required).map((field) => [field.key, required('Υποχρεωτικό πεδίο')]),
    )
  ...
  return <div className="hybrid-multi-selector">
    <div className="hybrid-multi-head">
      ...
      <div className="hybrid-multi-modes">
        <button type="button" className={mode === 'list' ? 'active' : ''} onClick={() => setMode('list')}>Από λίστα</button>
        <button type="button" className={mode === 'manual' ? 'active' : ''} onClick={() => setMode('manual')}>Χειροκίνητα</button>
      </div>
    </div>

    {mode === 'list' ? <>
      <label className="hybrid-multi-search"><Search size={15}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Αναζήτηση..." /></label>
      ...
      <button type="button" className="hybrid-multi-add" onClick={addManual}><Plus size={15}/> Προσθήκη</button>
    </div>}

    {selected.length > 0 && <div className="hybrid-multi-selected">
      {selected.map((item) => <span key={item.id}>{item.name}<button type="button" aria-label="Αφαίρεση" onClick={() => onChange(selected.filter((entry) => entry.id !== item.id))}><X size={13}/></button></span>)}
    </div>}
  </div>
}
